...
#model 2
fit_1_rt = forecast::hw(retail_trade_employment_ts, seasonal = "multiplicative", damped =T, h = 36)
fit_2_rt = forecast::hw(retail_trade_employment_ts, seasonal = "additive", damped = T, h = 36)
fit_3_rt = auto.arima(retail_trade_employment_ts)%>%
  forecast(h = 36)
...
final_model_vis = autoplot(final_model) + aes(color = "#ED3833", labels = "Historical Data") +
  ...
  labs(x= "", 
       y = "Employees (1000)", 
       subtitle = "Employment for retail trade is expected to peak at around 16,080 thousand employees",
       caption = "Forecasted using ARIMA(3,0,0)(2,0,0)[12] model. Data Retreived from FRED (CEU4200000001) on Nov 01, 2024") +
    #positions of title
  ...
  theme(axis.ticks.y = element_blank(),
    ...
    #start period
  annotate(geom = "point", x = 2024.917, y = 16060.43, colour = "red", size = 0.78)  
...
#model 2
fit_1_ae = forecast::hw(all_employment_ts, seasonal = "multiplicative", damped =T, h = 12)
fit_2_ae = forecast::hw(all_employment_ts, seasonal = "additive", damped = F, h = 12)
fit_3_ae = auto.arima(all_employment_ts)%>%
  forecast(h = 12)
fit_4_ae = forecast::ets(all_employment_ts)%>%
  forecast(h = 36)
...
final_model_vis = autoplot(final_model_all) + aes(color = "#ED3833", labels = "Historical Data") +
  ...
  theme(axis.ticks.y = element_blank(),
        ...
        legend.position = "none") +
    #start period
  annotate(geom = "point", x = 2024.917, y = 159184.0, colour = "red", size = 0.78)  
