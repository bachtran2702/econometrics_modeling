...
# Use model 3 because it performs best
csi_model = auto.arima(analysis_rdy_tsibble[,"consumer_sent"], stepwise = F) #AIC is high, lard se bands, more uncertainty
csi_model %>%
  forecast(h=12)%>%
  autoplot()

# flat lining rate at 3%, consumer sentiment is from 70 to 80 with an increase of 1 monthly
forcasted_stock = forecast(fit_sp, xreg = cbind(fed_fund_r = rep(3,12), unemployment_r= rep(4,12),consumer_sent =seq(70,82,1)))%>%
  autoplot()%>%
  summary()
```
